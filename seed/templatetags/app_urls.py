...
def urls_by_namespace(namespace, urlconf=None, args=None, kwargs=None, prefix=None, current_app=None):
    ...
    return dict((name, iri_to_uri(resolver._reverse_with_prefix(name, prefix, *args, **kwargs)))
                for name in resolver.reverse_dict.keys() if (isinstance(name, six.string_types) and 'datasets-detail' not in name and 'swagger' not in name and 'organizations-detail' not in name and 'organizations-shared' not in name and 'simple_api_test' not in name and 'firstvars' not in name and 'secondvars' not in name and 'users' not in name and 'projects' not in name and 'data_files' not in name))
